...
def _tts_cache_key(text: str) -> str:
    # Simple key; voice already fixed globally
    return text.strip().lower()

async def eleven_tts_stream_cached(text: str):
    # Try cache first by prefetching the whole audio into memory once
    ...
    # If not cached, stream and capture for next time
    chunks = []
    async for c in eleven_tts_stream(text):
        chunks.append(c)
        yield c

    # Store compacted chunks (avoid huge mem by trimming very long utterances)
    if len(b"".join(chunks)) < 2_000_000:  # ~2MB
...
async def warm_eleven():
    # fire-and-forget warm-up requests at startup
    try:
        for line in TTS_WARM_LINES:
            async for _ in eleven_tts_stream(line):
                break  # just force the first token path
    ...
    async def speak(text: str):
        ...
        async def _run():
            ...
            try:
                ...
                # inside speak(), replace: async for pcm in eleven_tts_stream(text):
